...
<head>
  ...
  <title>Your Favorites | HungryTime</title>
    ...
    <div class="main-body-wrapper">
      <section class="main-body-header">
        ...
        <div class="search-bar-wrapper">
          <form class="search-bar" method="POST" action="category-recipes.php?origin=favorites.php<?php
            if(isset($_GET['country'])) {
      ...
      <?php 
        $query = "SELECT * FROM favorites WHERE username = '$username' ORDER BY id DESC";
      ...
      <div class="category-recipes-wrapper">
        ...
        if(mysqli_num_rows($recipe_result)==0) {
          echo "<p><i>No recipes are added to your favorites.</i></p>";
        }
        
          while($faves_recipe_id_select = mysqli_fetch_assoc($recipe_result)) { 
            $faves_recipe_id = $faves_recipe_id_select['recipe_id'];

            $query = "SELECT * FROM recipes WHERE recipe_id = '$faves_recipe_id'";
            $recipe_select_result = mysqli_query($db, $query);

            $recipe = mysqli_fetch_assoc($recipe_select_result);
          ...
          <a href="recipe-main.php?recipe_id=<?php echo $recipe['recipe_id'];?>&origin=favorites.php&country=<?php echo $country;
            if(isset($_GET['time'])) {
              echo "&time=".$_GET['time'];
            }
            if(isset($_GET['category'])) {
        ...
        <div class="sidebar-user">
          ...
          <?php if($username=="Guest") { ?>
            ...
            <div class="sidebar-user-name">
              <p><a href="login.php?origin=favorites.php&country=<?php echo $country;?>&time=<?php echo $time;?>&category=<?php echo $category;?>&origin_2=<?php echo $origin;
                if(isset($_GET['origin_3'])) {
                ...
                if($search!=NULL){
                  echo "&search=".$search;
                }?>">Login</a>/<a href="login.php?signup=yes&origin=favorites.php&country=<?php echo $country;?>&time=<?php echo $time;?>&category=<?php echo $category;?>&origin_2=<?php echo $origin;
                if(isset($_GET['origin_2'])) {
                  echo "&origin_3=".$_GET['origin_2'];
                }
                if($search!=NULL){
          ...
          <?php } else { ?>
            ...
            <div class="sidebar-user-name sidebar-user-name-active">
              ...
              <a href="login.php?origin=favorites.php&country=<?php echo $country;?>&time=<?php echo $time;?>&category=<?php echo $category;?>&origin_2=<?php echo $origin;
                if(isset($_GET['origin_2'])) {
                  echo "&origin_3=".$_GET['origin_2'];
                }
                if($search!=NULL){
        ...
        <?php
          if($username!="Guest") { ?>
            <div class="sidebar-recently-viewed-wrapper">
              ...
              <a href="recipe-main.php?recipe_id=<?php echo $recently_viewed_recipe['recipe_id'];?>&origin=favorites.php<?php
        if(isset($_GET['country'])) {
